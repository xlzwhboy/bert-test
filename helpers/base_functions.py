...
def load_standard_query(inputfile):
    '''加载标准问库'''
    df=pd.read_csv('data/standard_query/'+inputfile,header=None)
    return df[0].values.tolist()

def load_user_query_from_train(inputfile,n):
    '''加载用户查询问题从训练集中'''
    df=pd.read_csv('data/'+inputfile,header=None)
    return list(df.head(n)[1].values)
...
def find_difference_of_semantic_similarity(mapfile,n, file_dir1, file_dir2):
    '''查找不同模型语义相似度测试结果的差异'''
    map={}
    ...
    files1=os.listdir(path+file_dir1)
    files2=os.listdir(path+file_dir2)

    df_res=pd.DataFrame(columns=['标准问','相似问','相似度-模型A','相似度-模型B','排名-模型A','排名-模型B'])
    query_sta=[]
    query_sim=[]
    sim_base_list=[]
    sim_tuned_list=[]
    ...
    for file in files1:
        df1=pd.read_csv(path+file_dir1+file,header=None)
        df2=pd.read_csv(path+file_dir2+file,header=None)
        s1=''
        s2=''
        sim_base=-1.0
        sim_tuned=-1.0
    ...
    df_res = df_res.append(pd.DataFrame(
        {'标准问':query_sta,'相似问':query_sim, '相似度-模型A':sim_base_list, '相似度-模型B': sim_tuned_list, '排名-模型A': rank_base_list, '排名-模型B': rank_tuned_list}))
    ...
    df_res.sort_values('标准问',inplace=True)
